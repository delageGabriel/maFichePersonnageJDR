...
namespace maFichePersonnageJDR.Model
{
    class ArmuresModel
    {
        ...
        public int GetArmuresIdByName(string nomArmure)
        {
            ...
            {
                // Commande
                SQLiteCommand command = new SQLiteCommand(string.Format("SELECT id_armure FROM ARMURES WHERE nom_armure = '{0}'", nomArmure), DatabaseConnection.Instance.GetConnection());
        ...
        /// <returns></returns>
        public string GetArmureNameById(int idArmure)
        {
            string nomArmure = string.Empty;

            try
            {
                SQLiteCommand command = new SQLiteCommand("SELECT nom_armure " +
                    "FROM ARMURES " +
                    "WHERE id_armure = @idArmure", DatabaseConnection.Instance.GetConnection());
